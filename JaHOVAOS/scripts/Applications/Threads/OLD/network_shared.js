//importScripts("http://10.0.1.15/~cclark/Sandbox/JaHOVA/JaHOVA/Omega/MT/scripts/jahova/OS/Cores/Network/socket_io/socket.io.js");
...
var Thread = {
    ...
    Initialize: function(){
        ...
        task.data = "Network iframe Loaded";
        task.threadID = -1;
        ...
        Thread.customCommands.put("CONNECT", JOSS.Connect);
        Thread.customCommands.put("DISCONNECT", JOSS.Disconnect);
        Thread.customCommands.put("GETLISTOFGAMES", JOSS.GetListOfGames);
        Thread.customCommands.put("JOINGAME", JOSS.JoinGame);
        Thread.customCommands.put("STARTGAME", JOSS.StartGame);
        Thread.customCommands.put("SENDCHAT", JOSS.SendChat);
        Thread.customCommands.put("UPDATEVELOCITY", JOSS.Update.Velocity);
        Thread.customCommands.put("UPDATETURRET", JOSS.Update.Turret);
        Thread.customCommands.put("UPDATEPOSITION", JOSS.Update.Position);
        Thread.customCommands.put("UPDATEGOLIATH", JOSS.Update.Goliath);
        Thread.customCommands.put("UPDATESTARFIRE", JOSS.Update.Starfire);
        Thread.customCommands.put("UPDATEASTEROID", JOSS.Update.Asteroid);
        Thread.customCommands.put("CREATEGAME", JOSS.Create.Game);
        Thread.customCommands.put("CREATEBULLET", JOSS.Create.Bullet);
        Thread.customCommands.put("CREATECANNONBALL", JOSS.Create.Cannonball);
        Thread.customCommands.put("CREATEASTEROID", JOSS.Create.Asteroid);
        Thread.customCommands.put("SETPROPERTY", JOSS.SetProperty);
        Thread.customCommands.put("GETSHAREDMEMORY", BroadcastPositionUpdate);
...
Update = function(){
    ...
    //if(JOSS.server && Omega.pilot && Omega.gameStarted){
...
BroadcastPositionUpdate = function(input){
    var data = {};
    data.msgType = "PositionUpdate";
    data.position = {};
    data.position.x = input.data[0].Position[0];
    data.position.y = input.data[0].Position[1];
    data.position.z = input.data[0].Position[2];
    data.speed = input.data[0].speed;
    data.yaw = input.data[0].yaw;
    data.pitch = input.data[0].pitch;
    JOSS.server.Broadcast(JOSS.gameID, input)
}
...
var CJOSServer = function(domain, port, path){
    ...
    this.socket = new CSocketIO(domain, port, path, 0);
...
JOSS = {
    ...
    gameID: null,
    Connect: function(){
        var domain = prompt("Enter Domain for Server");
        var port = prompt("Enter Port for Server");
        JOSS.server = new CJOSServer(domain, port, "");//os.network.Create.JOSServer("demo.jahovaos.com", "8081", "");
    ...
    SetProperty: function(input){
        JOSS.server.Set.SessionProperty(JOSS.gameID, input.key, input.value);
    },
    Update: {
        Velocity: function(input){
            //var data = {};
            //data.msgType = "VelocityUpdate";
            //data.speed = input.speed;
            //data.yawSpeed = input.yawSpeed;
            //data.pitchSpeed = input.pitchSpeed;
            //JOSS.server?JOSS.server.Broadcast(JOSS.gameID, data):data;
            JOSS.server.Broadcast(JOSS.gameID, input)
        },
        Turret: function(input){
            //var data = {};
            //data.msgType = "TurretUpdate";
            //data.turretYaw = input.data.turretYaw;
            JOSS.server.Broadcast(JOSS.gameID, input)
            //JOSS.server?JOSS.server.Broadcast(JOSS.gameID, data):data;
        },
        Position: function(input){
            ...
            //out.Position = [input.position.x, input.position.y, input.posiiton.z]
            //out.yaw = ent.yaw;
            //out.ptch = ent.pitch;
            //out.roll = ent.roll;
            //out.Physics = {};
            //out.Physics.speed = ent.Physics.speed;
            //out.Physics.yawSpeed = ent.Physics.yawSpeed;
            //out.Physics.pitchSpeed = ent.Physics.pitchSpeed;
            //
            //Thread.currentTask.taskType = "SetSharedMemoryEntry";
            //Thread.currentTask.data.key = out.id;
            //Thread.currentTask.data.value = out;
            //Thread.SendResponse(Thread.currentTask.Serialize());
    
            //var data = {};
            //data.msgType = "PositionUpdate";
            //data.position = {};
            //data.position.x = input.data.Position[0];
            //data.position.y = input.data.Position[1];
            //data.position.z = input.data.Position[2];
            ////data.speed = ironstar.Physics.speed;
            //data.yaw = input.data.yaw;
            //data.pitch = input.data.pitch;
            JOSS.server.Broadcast(JOSS.gameID, input)
        },
        Goliath: function(input){
            //var data = {};
    ...
    Create: {
        Game: function(input){
            ...
            input.data ==  "pilot" ? Omega.pilotID = JOSS.server.userID : Omega.gunnerID = JOSS.server.userID;
        },
        Bullet: function(){
            var data = {};
    ...
    onError: function(e){
        ...
        //Thread.currentTask.type = type;
        Thread.currentTask.taskType = "NO SERVER FOUND";
        Thread.currentTask.data.type = "singlePlayer";
        Thread.SendResponse(Thread.currentTask.Serialize());
    },
    onClose: function(){
    ...
    onMessage: function(msg){
        ...
        if(type == "sessionBroadcast"){
            Thread.currentTask.taskType = "Session Broadcast";
            Thread.currentTask.data = {};
            Thread.currentTask.data.type = type;
            Thread.currentTask.data.data = data;
            Thread.SendResponse(Thread.currentTask.Serialize());
            //if(data.msgType == "StartGame"){
            //    Omega.gameStarted = true;
            //    Update();
            //}
            ...
            Thread.currentTask.taskType = "Session List";
            Thread.currentTask.data ={};
            Thread.currentTask.data.type = type;
            Thread.currentTask.data.data = msg.Data.Args;
            Thread.SendResponse(Thread.currentTask.Serialize());
            ...
            Thread.currentTask.taskType = "Session Joined";
            Thread.currentTask.data ={};
            Thread.currentTask.data.type = type;
            Thread.currentTask.data.data = data;
            Thread.SendResponse(Thread.currentTask.Serialize());
            ...
            Thread.currentTask.taskType = "Session Created";
            Thread.currentTask.data ={};
            Thread.currentTask.data.type = type;
            Thread.currentTask.data.data = msg.Data.Args[0];
            Thread.SendResponse(Thread.currentTask.Serialize());
            //Save Game ID
            ...
            Thread.currentTask.taskType = "Property Set";
            Thread.currentTask.data ={};
            Thread.currentTask.data.type = type;
            Thread.currentTask.data.data = msg.Data.Args;
            Thread.SendResponse(Thread.currentTask.Serialize());
            ...
            Thread.currentTask.taskType = "Unknown Server Response";
            Thread.currentTask.data ={};
            Thread.currentTask.data.type = type;
            Thread.currentTask.data.data = msg.Data.Args;
            Thread.SendResponse(Thread.currentTask.Serialize());
