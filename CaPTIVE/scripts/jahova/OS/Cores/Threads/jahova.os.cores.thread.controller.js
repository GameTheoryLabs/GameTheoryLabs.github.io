...
var CThread = function(){
    ...
    this.ProcessTask = function(e){
        ...
        msg.osThreadID = response.threadID;
...
var ThreadController = {
    ...
    Initialize: function(){
        var msg = new CCommand();
        msg.data = "Thread Controller Online";
        
        self.addEventListener('message', ThreadController.ProcessRequest, false);
        ThreadController.NestedWorkers = true;
        self.postMessage(msg.Serialize());
    ...
    CreateThread: function(cmd){
        ...
        if(thread.type == "DYNAMIC")
        {
            ...
            thread.filename = os.GetDomain() + "scripts/jahova/OS/Cores/Threads/jahova.os.cores.thread.dynamic.js";
            ...
            //{
            //    task.data.input = properties.data.input;
                thread.status = "BUSY";
            //}
